  ...
  return (
    ...
    >
      <div className="editorial-container">
        ...
        <MobileStagger stagger={0.14}>
          {experience.map((entry) => (
            <MobileStaggerItem key={entry.company}>
              <article className="grid md:grid-cols-12 gap-4 md:gap-10 border-t border-[var(--color-line)] pt-10 mb-10">
        ...
        <div id="achievements" className="mt-20 md:mt-28 pt-16 border-t border-[var(--color-line)]">
          ...
          <MobileStagger className="grid sm:grid-cols-2 gap-x-16 gap-y-12" stagger={0.1}>
            {achievements.map((item) => (
              <MobileStaggerItem key={item.title}>
                <article>
